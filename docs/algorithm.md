...
> - 2026-09-24（第四轮）—— **桌面客户端**落地（`app/client/`，多端展示 P2）：§1 的代码层次表
>   与 §1.1 状态表各增一行。这一轮的实质是**把展示层补完到「能看见」**：此前
>   `app/present/` 只有规则没有界面。同时给 `hub` 补了写通道 `POST /hidden`（没有它，
>   D5 的隐藏开关无处上报，前端就是一个「点了没用」的控件），并在 `app/present/` 增加
>   三个纯函数（`color_for_key` / `columns_for` / `LABEL_TEXT`）—— 展示规则一律留在这层，
>   `app/client/` 只画不算。验收实测与冒烟口径见 `docs/reports/client-2026-09-24.md`。
...
| 界面呈现 | `app/client/`（Tkinter 外壳；规则全部来自 `app/present/`，本层只画） |
...
| `app/client/` | ✅ 已实现 | Tkinter 桌面外壳：一个窗口的展开态（宫格 / 教师汇总 / 隐藏开关）⇄ 最小化态（56×56 圆 + 气泡）。**纯 GUI，整体 `omit` 于覆盖率**；所有展示规则仍在 `app.present` |
...
⚠️ 关于 `app/client/` 的「已实现」要按**能跑的界面**理解，而不是「有测试保证」：
它在 CI 里被整体排除（headless 环境起不了 Tk），验证靠 `python -m app.client --demo --selftest`
这条冒烟通道 —— 它跑一遍「构建 → 绘制 → 切最小化 → 再展开」并打印统计。
因此**任何判定规则都不得写进 `app/client/`**：写进去既测不到，也让 CI 无感。
四色映射、0 人置灰、宫格列数、状态中文名、气泡分量这些规则都在 `app/present/`（100% 覆盖）。
