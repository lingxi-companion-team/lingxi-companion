...
#:
#: **标定方法**（可复现，脚本见 ``docs/reports/`` 的压测与标定报告）：
#: 取确定性随机噪声作「高频充足」的基准图，再连续施加 3×3 盒式模糊模拟失焦，
#: 记录每一遍之后的拉普拉斯方差：
#:
#: ==============  ================
#: 模糊遍数         拉普拉斯方差
...
#: ==============  ================
#:
#: 取 300 的含义是：**「连续两遍盒式模糊」正好落在容忍线附近** ——
#: 这正是「肉眼已能看出糊、但还没糊到不可用」的位置，与 ``blur_tolerance``
#: 的语义对得上。
#:
#: ⚠️ 这是**经验值**，且是**绝对**阈值（不随画面内容自适应）—— 换摄像头、
#: 换光照条件后必须重新标定。用规则法就是为了这一步能被快速做掉；
#: 换成学习模型后这个常数才会消失。
_LAP_VAR_REF = 300.0
